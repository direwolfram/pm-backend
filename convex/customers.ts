...
/**
 * Bounded per-customer reconciliation. While a reconciliation is in flight,
 * `reconcile_cursor`/`reconcile_totals` are set on the customer and the
 * stored totals must not be treated as authoritative; the final chunk swaps
 * the reconciled totals in atomically and clears the in-progress state.
 */
async function reconcileCustomerStatsChunk(
  ...
  const stored = customer.reconcile_cursor;
  const effectiveCursor = cursor ?? stored ?? null;
  const result = await ctx.db
    .query("orders")
    .withIndex("by_customer", (q) => q.eq("customer_id", customerId))
    .order("asc")
    .paginate({ numItems: 100, cursor: effectiveCursor });
  const running = customer.reconcile_totals ?? { order_count: 0, total_spend: 0 };
...
export const backfillCustomerOrderStats = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const customer of customers) {
      ...
      // per-customer order scans run in their own continuations).
